...
class Command(BaseCommand):
    help = (
        "Seed the database with fixture data, creating organization schemas as needed."
    )
    ...
    def handle(self, *args, **options):
        ...
        for entry in orgs_data:
            ...
            for membership_data in fields.get("memberships", []):
                ...
                self.stdout.write(
                    f"    {user} -> {org} ({membership_data.get('role', 'member')})",
                )
        ...
        for org in Organization.objects.all():
            ...
            if fixture_file.exists():
                ...
                self.stdout.write(
                    f"  No fixture found for {org.schema_name}, skipping.",
                )
