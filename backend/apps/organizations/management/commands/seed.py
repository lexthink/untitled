    ...
    def handle(self, *args, **options):
        ...
        for entry in orgs_data:
            ...
            if created:
                ...
                self.stdout.write(f"  Already exists: {organization}")
            ...
            for membership_data in fields.get("memberships", []):
                ...
                self.stdout.write(
                    f"    {user} -> {organization} ({membership_data.get('role', 'member')})",
                )

        # 3. Load organization-specific data
        for organization in Organization.objects.all():
            fixture_file = fixtures_dir / f"{organization.schema_name}.json"
            if fixture_file.exists():
                self.stdout.write(f"Loading data for {organization.schema_name}...")
                with tenant_context(organization):
                    call_command("loaddata", str(fixture_file))
            else:
                self.stdout.write(
                    f"  No fixture found for {organization.schema_name}, skipping.",
                )
